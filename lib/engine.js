...
class Engine {
  ...
  _runCondition( { condition, doc, failures, passes } ) {
    if( condition.all || condition.any ) {
      if( condition.all ) {
        condition.all.map( condition => this._runCondition( { condition, doc, failures, passes } ) );
        condition.success = condition.all.reduce( (accum, current) => accum ? current.success : false, true );
      } else if( condition.any ) {
        condition.any.map( condition => this._runCondition( { condition, doc, failures, passes } ) );
  ...
  _runRule( { rule, doc, failures, passes } ) {
    if( rule.conditions.all ) {
      rule.conditions.all.map( condition => this._runCondition( { condition, doc, failures, passes } ) );
      rule.success = rule.conditions.all.reduce( (accum, current) => accum ? current.success : false, true );
    }
    else if( rule.conditions.any ) {
      rule.conditions.any.map( condition => this._runCondition( { condition, doc, failures, passes } ) );
  ...
  run( doc ) {
    ...
    const failures = [];
    const passes = [];
    rules.map( rule => this._runRule( { rule, doc, failures, passes } ) );
    const result = { rules };
